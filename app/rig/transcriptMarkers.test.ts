...
describe("parseTranscriptSegments", () => {
  it("returns a single text segment when there are no markers", () => {
    expect(parseTranscriptSegments("What does this mean?")).toEqual([
      { type: "text", value: "What does this mean?" },
    ]);
  });
  ...
  it("splits a context tag, a pill tag, and surrounding text", () => {
    ...
    expect(parseTranscriptSegments(text)).toEqual([
      {
        type: "context",
        text: 'Reading "Capital" by Karl Marx. Currently on screen:\n\nA commodity...',
      },
      { type: "text", value: "\n\nWhat does " },
      {
        type: "pill",
    ...
    expect(parseTranscriptSegments(text)).toEqual([
      {
        ...
        kind: "note",
        locator: "§2 ¶1",
        text: 'Is "value" the right word here?',
      },
    ]);
  });
  ...
  it("leaves the old, pre-marker formats as plain text", () => {
    ...
    expect(parseTranscriptSegments(oldPill)).toEqual([
      { type: "text", value: oldPill },
    ]);

    const oldContext = `[Reading "Capital" by Karl Marx. Currently on screen:\n\nA commodity...]`;
    expect(parseTranscriptSegments(oldContext)).toEqual([
      { type: "text", value: oldContext },
    ]);
  });
