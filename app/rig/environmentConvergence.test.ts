...
describe("environmentMatchesConfig", () => {
  it("matches an unchanged environment", () => {
    expect(environmentMatchesConfig(matchingLiveEnvironment(), buildEnvironmentConfig())).toBe(true);
  });

  it("detects a name change", () => {
    const live = matchingLiveEnvironment();
    live.name = "Something Else";
    expect(environmentMatchesConfig(live, buildEnvironmentConfig())).toBe(false);
  });
  ...
  it("detects switching away from a cloud config", () => {
    const live = matchingLiveEnvironment();
    (live as { config: Anthropic.Beta.Environments.BetaEnvironment["config"] }).config = { type: "self_hosted" };
    expect(environmentMatchesConfig(live, buildEnvironmentConfig())).toBe(false);
  });

  it("detects networking opening up beyond limited", () => {
    const live = matchingLiveEnvironment();
    if (live.config.type !== "cloud") throw new Error("expected a cloud config");
  ...
  it("detects an allowed host being added", () => {
    const live = matchingLiveEnvironment();
    if (live.config.type !== "cloud" || live.config.networking.type !== "limited") {
  ...
  it("ignores allowed_hosts ordering", () => {
    const config = buildEnvironmentConfig();
    if (config.config?.type !== "cloud" || config.config.networking?.type !== "limited") {
      throw new Error("expected limited networking");
    }
