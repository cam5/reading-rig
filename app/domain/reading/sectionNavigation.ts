...
export type ChapterOutline = { id: string; ordinal: number; sections: SectionOutline[] };
 ...
 */
export function resolveSectionRef(chapters: ChapterOutline[], sectionId?: string | null): SectionRef | null {
  if (sectionId) {
  ...
  return firstChapter && firstSection ? { chapterId: firstChapter.id, sectionId: firstSection.id } : null;
}
 ...
 */
export function nextSectionRef(chapters: ChapterOutline[], current: SectionRef): SectionRef | null {
  const chapterIndex = chapters.findIndex((c) => c.id === current.chapterId);
  if (chapterIndex === -1) return null;
  const chapter = chapters[chapterIndex];
  const sectionIndex = chapter.sections.findIndex((s) => s.id === current.sectionId);
  if (sectionIndex === -1) return null;

  if (sectionIndex + 1 < chapter.sections.length) {
    return { chapterId: chapter.id, sectionId: chapter.sections[sectionIndex + 1].id };
  }
  const nextChapter = chapters[chapterIndex + 1];
  const nextSection = nextChapter?.sections[0];
  return nextChapter && nextSection ? { chapterId: nextChapter.id, sectionId: nextSection.id } : null;
}

/** The mirror of {@link nextSectionRef}: one step back, rolling into the
 * previous chapter's last section rather than its first. */
export function previousSectionRef(chapters: ChapterOutline[], current: SectionRef): SectionRef | null {
  const chapterIndex = chapters.findIndex((c) => c.id === current.chapterId);
  if (chapterIndex === -1) return null;
  const chapter = chapters[chapterIndex];
  const sectionIndex = chapter.sections.findIndex((s) => s.id === current.sectionId);
  ...
  const previousSection = previousChapter?.sections[previousChapter.sections.length - 1];
