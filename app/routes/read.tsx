...
export async function action({ request }: Route.ActionArgs) {
  ...
  // Selected down to just what each intent needs, including the workId
  // chain the bookmark intent uses.
  const paragraph = await db.paragraph.findFirst({
    where: { id: paragraphId, section: { chapter: { work: { userId: user.id } } } },
    select: { id: true, section: { select: { chapter: { select: { workId: true } } } } },
  });
